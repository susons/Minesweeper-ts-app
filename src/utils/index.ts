...
const grabAllNearByCells = (cells: Cell[][], rowParam: number, colParam: number): {
  ...
  for (let rowIndex = 0; rowIndex < MAX_ROWS; rowIndex++) {
    for (let colIndex = 0; colIndex < MAX_COLS; colIndex++) {
      ...
      const {
        ...
        bottomCell,
        bottomRightCell
      } = grabAllNearByCells(cells, rowIndex, colIndex);
...
export const openMultipleCells = (cells: Cell[][], rowParam: number, colParam: number): Cell[][] => {
  const {
    ...
    bottomCell,
    bottomRightCell,
  } = grabAllNearByCells(cells,rowParam,colParam);
  const currentCell = cells[rowParam][colParam];
  
  if (currentCell.state === CellState.visible || currentCell.state === CellState.flagged) return cells;
  ...
  if (topLeftCell?.state === CellState.open && topLeftCell.value !== CellValue.bomb) {
    ...
    } else newCells[rowParam - 1][colParam - 1].state = CellState.visible;
  }

  if (topCell?.state === CellState.open && topCell.value !== CellValue.bomb) {
    if (topCell.value === CellValue.none) {
      openMultipleCells(newCells, rowParam - 1, colParam)
    } else newCells[rowParam - 1][colParam].state = CellState.visible;
  }
  ...
  if (leftCell?.state === CellState.open && leftCell.value !== CellValue.bomb) {
    ...
    } else newCells[rowParam][colParam - 1].state = CellState.visible;
  }
